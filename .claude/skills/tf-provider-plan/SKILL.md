...
2. Parse `$ARGUMENTS` for the resource name, provider name, and description. Ask via `AskUserQuestion` if incomplete — step 4 needs `<resource-name>` and step 8 needs `$RESOURCE`.
